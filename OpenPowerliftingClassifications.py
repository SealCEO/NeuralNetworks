#Please run OpenPowerliftingCleanUp.py before this
import pandas as pd
...
print("Run the OpenPowerliftingCleanUp.py beforehand and use that csv for this. DO NOT USE THE ORIGINAL.")
...
data = pd.read_csv('CleanedUpPowerlifting.csv')
data.astype(float)
X = data.drop(['Place'], axis = 1)
y = data[['Place']].values.ravel()
cv=5
    ...
    if True:
        ...
        if True:
            ...
            for num in list_of_nur:
                dict = {0 : nn.Sequential(nn.Linear(size, 2 * num), nn.SELU(), nn.Linear(2 * num, num), nn.SELU(), nn.Linear(num, classes), nn.Softmax(dim=1)),
                        1 : nn.Sequential(nn.Linear(size, num), nn.SELU(), nn.Linear(num, num), nn.SELU(), nn.Linear(num, classes), nn.Softmax(dim=1)),

                        2 : nn.Sequential(nn.Linear(size, 2 * num), nn.ReLU(), nn.Linear(2 * num, num), nn.ReLU(), nn.Linear(num, classes), nn.Softmax(dim=1)),
                        3 : nn.Sequential(nn.Linear(size, num), nn.ReLU(), nn.Linear(num, num), nn.ReLU(), nn.Linear(num, classes), nn.Softmax(dim=1)),

                        4 : nn.Sequential(nn.Linear(size, 2 * num), nn.Tanh(), nn.Linear(2 * num, num), nn.Tanh(), nn.Linear(num, classes), nn.Softmax(dim=1)),
                        5 : nn.Sequential(nn.Linear(size, num), nn.Tanh(), nn.Linear(num, num), nn.Tanh(), nn.Linear(num, classes), nn.Softmax(dim=1)),

                        6 : nn.Sequential(nn.Linear(size, 2 * num), nn.Softmax(dim=1), nn.Linear(2 * num, num), nn.Softmax(dim=1), nn.Linear(num, classes), nn.Softmax(dim=1)),
                        7 : nn.Sequential(nn.Linear(size, num), nn.Softmax(dim=1), nn.Linear(num, num), nn.Softmax(dim=1), nn.Linear(num, classes), nn.Softmax(dim=1)),

                        8 : nn.Sequential(nn.Linear(size, 2 * num), nn.Sigmoid(), nn.Linear(2 * num, num), nn.Sigmoid(), nn.Linear(num, classes), nn.Softmax(dim=1)),
                        9 : nn.Sequential(nn.Linear(size, num), nn.Sigmoid(), nn.Linear(num, num), nn.Sigmoid(), nn.Linear(num, classes), nn.Softmax(dim=1))}
        ...
        if True:
            ...
            model = NeuralNetwork(nn.Sequential(nn.Linear(11, 10), nn.SELU(), nn.Linear(11,4), nn.Softmax(dim=1)))
            if torch.cuda.device_count() > 1:
                model = nn.DataParallel(model)
